<aside
    ...
    class="fixed lg:static z-40 inset-y-0 left-0 w-64 bg-ink-900 text-white transition-transform duration-200 flex flex-col">
    ...
    <div class="h-16 px-5 flex items-center gap-3 border-b border-white/10 shrink-0">
        ...
        <button class="ml-auto lg:hidden p-1 rounded-lg hover:bg-white/10" @click="$store.ui.sidebarOpen = false">
    ...
    <nav class="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto scrollbar-thin">
        ...
        <div class="pt-4 pb-1">
            <div class="px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-white/30">Akun Admin</div>
        </div>
        @foreach([
            ['admin.profile',  'Profil Saya',  'user'],
    ...
    <div class="p-3 border-t border-white/10 space-y-0.5 shrink-0">
        ...
        <form method="POST" action="{{ route('logout') }}">@csrf
            <button class="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm font-medium text-white/70 hover:bg-white/10 hover:text-white transition text-left">
